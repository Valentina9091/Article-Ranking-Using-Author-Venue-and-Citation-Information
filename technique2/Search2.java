...
import org.apache.hadoop.util.ToolRunner;
	...
	public class Search2 extends Configured implements Tool {
		
		static final Logger LOG = Logger.getLogger(Search2.class.getName());
		public static void main(String[] args) throws Exception {
			Configuration conf= new Configuration(true);
			@SuppressWarnings("unused")
			int res = ToolRunner.run(conf,new Search2(), args);
		}
	...
	public void map(LongWritable offset, Text lineText, Context context)
		...
		for(String record:recArr){	
			...
			//LOG.info("2nddd mappp"+record);
			String record3[]=record.split("\t");
			//LOG.info("HERRRRRRRRr"+record3[0]);
			//LOG.info("HERRRRRRRRr2222222222222"+record3[1]);
			if((StringUtils.countMatches(record3[1], Constant.SEPARATOR)>2))
			{
			String record2[]=record3[1].split(Constant.SEPARATOR);
		//	LOG.info(record3[1]+"INNNNNNNNNNNNNNNNNNNNNNN");
			...
			{
			//	LOG.info(record3[1]+"INNNNNNNNNNNNNNNNNNNNNNN222222222222222222222222222");
